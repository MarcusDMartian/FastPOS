...
export const Header: React.FC = () => {
  ...
  return (
    <div className="fixed top-0 w-full z-50 flex flex-col font-sans transition-all duration-500">
      {/* === TOP BAR (Menubar) === */}
      <div
        className={`bg-gray-50 bg-subtle-pattern text-gray-600 text-xs transition-all duration-500 ease-in-out border-b border-gray-200 overflow-hidden ${scrolled ? 'max-h-0 opacity-0' : 'max-h-12 opacity-100 py-2.5'
          }`}
      >
      ...
      {/* Mobile Nav Drawer */}
      <div
        className={`fixed inset-0 z-40 bg-white transform transition-transform duration-500 cubic-bezier(0.4, 0, 0.2, 1) xl:hidden ${isOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
      >
        <div className="flex flex-col h-full p-8 bg-white bg-subtle-pattern">
